        ...
        if form.is_valid():
            ...
            try:
                with transaction.atomic():
                    # Save user but inactive until OTP verified
                    user = form.save(commit=False)
                    user.is_active = False
                    user.save()

                    # Create default profiles
                    if user.user_type == 'vendor' and not hasattr(user, 'vendorprofile'):
                        VendorProfile.objects.create(
                            user=user,
                            business_name='Pending - update your business name',
                            business_address='Pending - update your business address',
                            default_momo_number='0000000000',
                        )
                    elif user.user_type == 'customer' and not hasattr(user, 'customerprofile'):
                    ...
                    # Send OTP email
                    subject = "Verify Your SokHub Account"
                    message = f"Hello {user.username}, your OTP code is {otp.code}. It expires in 10 minutes."
...
def verify_otp(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if otp.code == input_code and timezone.now() <= expiry_time:
            user.is_active = True
            user.save()
            otp.is_verified = True
            otp.save()
            messages.success(request, "Your account has been verified successfully!")
            return redirect_user_by_role(user)  # call directly, no import needed
        else:
