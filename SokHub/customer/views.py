...
from .models import User, VendorProfile, CustomerProfile, create_user_profile, save_user_profile
...
def register(request):
    ...
    To avoid post_save signal creating profiles with missing required fields,
    we temporarily disconnect the profile-creation signals, save the user,
    then create safe placeholder profiles and reconnect signals.
    """
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # If disconnect fails (e.g., they aren't connected), continue
                logger.debug("Profile post_save receivers were not connected or failed to disconnect.")
            ...
            try:
                with transaction.atomic():
                    ...
                    # At this point signals are disconnected so no automatic profile creation happened.
                    # We create safe default profiles now to satisfy DB constraints.
                    if user.user_type == 'vendor':
                        # Create VendorProfile with safe default placeholders if not already present.
                        if not hasattr(user, 'vendorprofile'):
                            VendorProfile.objects.create(
                                user=user,
                                business_name='Pending - update your business name',
                                business_address='Pending - update your business address',
                                default_momo_number='0000000000',  # placeholder momo
                            )
                    elif user.user_type == 'customer':
                        if not hasattr(user, 'customerprofile'):
                            CustomerProfile.objects.create(
                                user=user,
                                shipping_address='Pending - add your shipping address'
                            )

                    # Reconnect signal handlers (do this before sending emails or logging)
                    try:
                        model_post_save.connect(create_user_profile, sender=User)
                        model_post_save.connect(save_user_profile, sender=User)
                    except Exception:
                        logger.warning("Failed to reconnect profile post_save receivers. Please check receiver names.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                logger.debug("Failed to send security notification email; continuing login flow.")
